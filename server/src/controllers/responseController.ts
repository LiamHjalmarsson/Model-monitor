...
export async function createResponse(req: AuthenticatedRequest, res: Response) {
	...
	try {
		...
		if (check.rowCount === 0) {
			return res.status(403).json({ message: "Not authorized to add response to this brand" });
		}
...
export async function generateAIResponse(req: AuthenticatedRequest, res: Response) {
	...
	try {
		...
		if (check.rowCount === 0) {
			return res.status(403).json({ message: "Not authorized to generate for this brand" });
		}
		...
		if (!prompt) {
			return res.status(400).json({ message: 'Brand prompt not found' });
		}
